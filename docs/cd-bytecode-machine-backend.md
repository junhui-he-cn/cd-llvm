...
enum-variant, bounded native-call including `contains`, `slice`, `copy`, `concat`, `push`, `pop`, `remove`, `clear`, `merge`, `keys`, and `values`, dynamic CD `select`/PHI, and `map`/`filter`/`flatMap`/`reduce`/`any`/`all`/`count`/`find`/`findIndex` callback-native slices share the
...
existing `cdbc 0.2` artifact.  It remains a design gate while the current
...
- The machine path is opt-in and may emit only CD pseudo-instructions with a
  stable `cdbc 0.2` mapping.  Lowering pseudos such as `CD_SELECT` expand only
  ...
  does not own the `.cdbc 0.2` wire spelling.
...
proven-dynamic-CD storage through the context-specific `load_local` /
`bind_local` / `set_local` and `load_global` / `init_global` / `set_global`
operations, conditional and unconditional branches,
...
`CD_LOAD_VAR` pseudo, which is serialized as a context-specific `load_local` or
`load_global`, and a marked direct call result or pointer
...
the result inherits that provenance. The direct path expands it to 0.2
`br_if`/`move`/`br` block instructions; the machine `CD_SELECT` pseudo bridges
to the same artifact sequence. Foreign or ordinary pointer arms and
...
paths. Both paths lower the slot through the 0.2 numeric local/global cell
operations. Escaped allocas, GEP/bitcast aliases, uninitialized or
...
The array-access slice adds `CD_INDEX` and `CD_LEN`. Their operands stay in the
`CDValue` virtual register class, and the bridge emits the 0.2 `index` and
`len` artifact operations after validating that every opaque pointer operand
came from an explicit CD intrinsic. `CD_ASSERT_ARRAY` remains a parser-visible
diagnostic boundary for the old intrinsic, but has no 0.2 artifact lowering and
is rejected at compile time. Scalar index constants are materialized before
`CD_INDEX`, so the machine path preserves the direct path's
...
`CD_VARIANT_FIELD`. The bridge collects numeric type and variant layouts and
emits `make_variant`, `is_variant`, and `variant_get`; payload, value, and
result operands remain `CDValue` virtual registers, while the payload index is
an immediate and the inferred type/variant IDs are serialized in the access
instructions. This preserves payload order and the Rust VM's
false/non-variant and bounds-error behavior. Payload registers are
materialized before `CD_VARIANT` is inserted, so nested explicit CD
constructors retain definition-before-use ordering. Ordinary pointers and
aggregates remain rejected by the shared ABI validator.

The bounded native-call slice covers `floor`, `ceil`, `sqrt`, `str`, `typeOf`,
`hash`, `contains`, `slice`, `copy`, `concat`, `push`, `pop`, `remove`, `clear`, `merge`, `keys`, `values`, `range`, `substr`, `charAt`, and the callback helpers `map`, `filter`,
`flatMap`, `reduce`, `any`, `all`, `count`, `find`, and `findIndex`. Their
native import index is an immediate machine operand, while arguments and the
result remain in the `CDValue` virtual register class. The `CD_NATIVE_CALL`
bridge emits the 0.2 `call_native iN` artifact operation and its
`native_imports` table entry after shared
...
For `push`, the machine lowerer accepts a proven CD dynamic-value array, a
scalar or proven CD dynamic-value value, and an exact address-space-zero `ptr`
result. The Rust VM owns the runtime array check, append mutation, resource
checkpoint, and nil result; ordinary pointer substitutes remain rejected.

For `pop`, the machine lowerer accepts a proven CD dynamic-value array and an
exact address-space-zero `ptr` result. The Rust VM owns the runtime array check,
last-element mutation, removed-value return, and empty-array diagnostic;
ordinary pointer substitutes remain rejected.

For `remove`, the machine lowerer accepts a proven CD dynamic-value map, a
scalar or proven CD dynamic-value key, and an exact address-space-zero `ptr`
result. The Rust VM owns map/key validation, first-match mutation,
removed-value return, and missing-key errors; ordinary pointer substitutes
remain rejected.

For `clear`, the machine lowerer accepts a proven CD dynamic-value map and an
exact address-space-zero `ptr` result. The Rust VM owns the runtime map check,
shared map mutation, and nil result; ordinary pointer substitutes remain
rejected.

For `merge`, the machine lowerer accepts two proven CD dynamic-value maps and
an exact address-space-zero `ptr` result. The Rust VM owns fresh allocation,
left-to-right insertion order, and right-side duplicate replacement while
retaining the first key position; ordinary pointer substitutes remain
rejected.
...
the only currently supported operations that have stable `cdbc 0.2`
representations.
...
| `CD_STRUCT` | `MakeStruct` -> `make_struct` | destination value, resolved type ID, variadic field values |
| `CD_VARIANT` | `MakeVariant` -> `make_variant` | destination value, resolved type ID, variant ID, variadic payload values |
| `CD_VARIANT_TAG` | `IsVariant` -> `is_variant` | destination value, value, resolved type ID, variant ID |
| `CD_VARIANT_FIELD` | `VariantGet` -> `variant_get` | destination value, value, inferred type ID, variant ID, immediate payload index |
...
| `CD_LEN` | `Len` -> `len` | destination value, collection value |
| `CD_MOVE` | `Move` | destination value, source value |
| `CD_LOAD_VAR` | `LoadLocal`/`LoadGlobal` -> `load_local`/`load_global` | destination value, resolved numeric local/global slot |
| `CD_STORE_VAR` | `BindLocal`/`SetLocal`/`InitGlobal`/`SetGlobal` -> numeric cell op | resolved numeric slot, source value |
| `CD_CALL` | `Call` | destination value, callee value, variadic arguments |
| `CD_NATIVE_CALL` | `CallNative` -> `call_native` | destination value, native-import index, variadic arguments |
| `CD_PRINT` | `CallNative` -> `call_native` | scratch destination, `print` native-import index, source value |
...
| `CD_JUMP` | `Br` -> `br` | symbolic target block label |
| `CD_JUMP_IF_FALSE` | `BrIf` -> `br_if` | condition value, true/false symbolic block labels |
| `CD_SELECT` | `br_if` + `move` + `br` + `move` | destination value, condition value, true value, false value |
