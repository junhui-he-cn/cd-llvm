...
| M4 | Explicit CD values: strings, arrays, maps, records, variants, indexing/mutation, bounded natives including `contains`/`slice`/`copy`/`concat`/`push`/`pop`/`remove`/`clear`/`merge`/`keys`/`values`, selected `map`/`filter`/`flatMap`/`reduce`/`any`/`all`/`count`/`find`/`findIndex` callbacks | Complete for the implemented bounded ABI | Dynamic CD `select`, PHI, and one-slot storage are supported through existing control-flow and variable operations; future native names stay rejected until separately selected |
...
| M7-local | Reproducible LLVM-only, VM, parity, and module-link verification | Complete | Latest local gate: 126 lit (125 passed, 1 unsupported), parity 95/95, VM `73 + 3 + 8`, module-link direct/machine passed |
...
### Narrow M4 follow-up: bounded native `concat` (2026-08-10)

**Goal:** Admit the Rust VM's existing `concat` helper through the shared
`llvm.cd.native` ABI without adding a wire operation, artifact field, or
changing the nested VM.

The accepted shape is two proven CD dynamic-value tokens and an exact
address-space-zero `ptr` result. The VM owns the runtime array checks, snapshot
order, and fresh shallow-array allocation; ordinary LLVM pointers remain
rejected by shared provenance validation.

- [x] Add the name-specific capability matrix to `CDValueABI` and the typed
  artifact native-name allowlist.
- [x] Add positive empty/non-empty ordered concatenation, runtime non-array,
  and malformed direct/machine fixtures.
- [x] Add behavior/runtime-error entries to the direct/machine parity manifest.
- [x] Update the ABI, machine-backend, target README, verification matrix, and
  active development plan.

Verification on 2026-08-10: focused concat/error lit passed `3/3`; the full
local CD suite passed `110` tests with `109` passed and `1` expected unsupported
VM case; direct/machine parity passed `79/79`; parity unit tests passed `14/14`,
module-link unit tests passed `5/5`, module-link direct/machine integration
passed, Rust VM tests passed `73 + 3 + 8`, and the nested checkout remained
clean.

### Narrow M4 follow-up: bounded native `keys` (2026-08-10)

**Goal:** Admit the Rust VM's existing `keys` helper through the shared
`llvm.cd.native` ABI without adding a wire operation, artifact field, or
changing the nested VM.

The accepted shape is a proven CD dynamic-value token and an exact
address-space-zero `ptr` result. The VM owns the runtime map check,
insertion-order snapshot, and fresh array allocation; ordinary LLVM pointers
remain rejected by shared provenance validation.

- [x] Add the name-specific capability matrix to `CDValueABI` and the typed
  artifact native-name allowlist.
- [x] Add positive empty/non-empty insertion-ordered key arrays, runtime
  non-map, and malformed direct/machine fixtures.
- [x] Add behavior/runtime-error entries to the direct/machine parity manifest.
- [x] Update the ABI, machine-backend, target README, verification matrix, and
  active development plan.

Verification on 2026-08-10: focused keys/error lit passed `3/3`; the full local
CD suite passed `112` tests with `111` passed and `1` expected unsupported VM
case; direct/machine parity passed `81/81`; parity unit tests passed `14/14`,
module-link unit tests passed `5/5`, module-link direct/machine integration
passed, Rust VM tests passed `73 + 3 + 8`, and the nested checkout remained
clean.

### Narrow M4 follow-up: bounded native `values` (2026-08-10)

**Goal:** Admit the Rust VM's existing `values` helper through the shared
`llvm.cd.native` ABI without adding a wire operation, artifact field, or
changing the nested VM.
...
- [x] Add the name-specific capability matrix to `CDValueABI` and the typed
  artifact native-name allowlist.
- [x] Add positive empty/non-empty insertion-ordered value arrays, runtime
  non-map, and malformed direct/machine fixtures.
- [x] Add behavior/runtime-error entries to the direct/machine parity manifest.
- [x] Update the ABI, machine-backend, target README, verification matrix, and
  active development plan.

The positive fixture is `cdbc-native-map-values.ll` because the existing
`cdbc-native-values.ll` artifact fixture continues to cover `str`, `typeOf`,
and `range` native lowering. Focused values/error lit passed `3/3`; the full
local CD suite passed `114` tests with `113` passed and `1` expected unsupported
VM case; direct/machine parity passed `83/83`; parity unit tests passed `14/14`;
module-link unit tests passed `5/5`; module-link direct/machine integration
passed; Rust VM tests passed `73 + 3 + 8`; and `git diff --check` passed.

### Narrow M4 follow-up: bounded native `remove` (2026-08-10)

**Goal:** Admit the Rust VM's existing `remove` helper through the shared
`llvm.cd.native` ABI without adding a wire operation, artifact field, or
changing the nested VM.

The accepted shape is a proven CD dynamic-value map, a scalar or proven
CD dynamic-value key, and an exact address-space-zero `ptr` result. The VM
owns map/key validation, first-match mutation, removed-value return, and
missing-key errors; ordinary LLVM pointers remain rejected by shared
provenance validation.

- [x] Add the name-specific capability matrix to `CDValueABI` and the typed
  artifact native-name allowlist.
- [x] Add positive scalar/CD key removal, shared-map mutation, runtime
  non-map, and malformed direct/machine fixtures.
- [x] Add behavior/runtime-error entries to the direct/machine parity manifest.
- [x] Update the ABI, machine-backend, target README, verification matrix, and
  active development plan.
...
### Narrow M4 follow-up: bounded native `merge` (2026-08-10)

**Goal:** Admit the Rust VM's existing `merge` helper through the shared
`llvm.cd.native` ABI without adding a wire operation, artifact field, or
changing the nested VM.

The accepted shape is two proven CD dynamic-value maps and an exact
address-space-zero `ptr` result. The VM owns fresh allocation, left-to-right
insertion order, and right-side duplicate replacement while retaining the
first key position; ordinary LLVM pointers remain rejected by shared
provenance validation.

- [x] Add the name-specific capability matrix to `CDValueABI` and the typed
  artifact native-name allowlist.
- [x] Add fresh ordered-map behavior, duplicate-key replacement, both runtime
  map errors, and malformed direct/machine fixtures.
- [x] Add behavior/runtime-error entries to the direct/machine parity manifest.
- [x] Update the ABI, machine-backend, target README, verification matrix, and
  active development plan.

Focused merge/error lit passed `4/4`; the full local CD suite passed `121`
tests with `120` passed and `1` expected unsupported VM case; direct/machine
parity passed `90/90`; parity unit tests passed `14/14`; module-link unit tests
passed `5/5`; module-link direct/machine integration passed; Rust VM tests
passed `73 + 3 + 8`; and `git diff --check` passed.

### Narrow M4 follow-up: bounded native `clear` (2026-08-10)

**Goal:** Admit the Rust VM's existing `clear` helper through the shared
`llvm.cd.native` ABI without adding a wire operation, artifact field, or
changing the nested VM.

The accepted shape is a proven CD dynamic-value map and an exact
address-space-zero `ptr` result. The VM owns the runtime map check, shared map
mutation, and nil result; ordinary LLVM pointers remain rejected by shared
provenance validation.

- [x] Add the name-specific capability matrix to `CDValueABI` and the typed
  artifact native-name allowlist.
- [x] Add positive in-place map clearing, nil result, runtime non-map, and
  malformed direct/machine fixtures.
- [x] Add behavior/runtime-error entries to the direct/machine parity manifest.
- [x] Update the ABI, machine-backend, target README, verification matrix, and
  active development plan.

Focused clear/error lit passed `3/3`; the full local CD suite passed `118`
tests with `117` passed and `1` expected unsupported VM case; direct/machine
parity passed `87/87`; parity unit tests passed `14/14`; module-link unit tests
passed `5/5`; module-link direct/machine integration passed; Rust VM tests
passed `73 + 3 + 8`; and `git diff --check` passed.

### Narrow M4 follow-up: bounded native `pop` (2026-08-10)

**Goal:** Admit the Rust VM's existing `pop` helper through the shared
`llvm.cd.native` ABI without adding a wire operation, artifact field, or
changing the nested VM.

The accepted shape is a proven CD dynamic-value array and an exact
address-space-zero `ptr` result. The VM owns the runtime array check,
last-element mutation, removed-value return, and empty-array error; ordinary
LLVM pointers remain rejected by shared provenance validation.

- [x] Add the name-specific capability matrix to `CDValueABI` and the typed
  artifact native-name allowlist.
- [x] Add positive last-element return/shared mutation, runtime non-array and
  empty-array failures, and malformed direct/machine fixtures.
- [x] Add behavior/runtime-error entries to the direct/machine parity manifest.
- [x] Update the ABI, machine-backend, target README, verification matrix, and
  active development plan.

Focused pop/error lit passed `4/4`; the full local CD suite passed `126` tests
with `125` passed and `1` expected unsupported VM case; direct/machine parity
passed `95/95`; parity unit tests passed `14/14`; module-link unit tests passed
`5/5`; module-link direct/machine integration passed; Rust VM tests passed
`73 + 3 + 8`; and `git diff --check` passed.
