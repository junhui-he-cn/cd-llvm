...
- Modify: `llvm/lib/Target/CD/CDValueABI.cpp` and
  `llvm/lib/Target/CD/CDBytecodeFormat.cpp`.
- Create: `llvm/test/CodeGen/CD/cdbc-native-find.ll` and
  `llvm/test/CodeGen/CD/cdbc-native-find-runtime.ll`.
- Modify: `llvm/test/CodeGen/CD/cdbc-native-predicate-errors.ll` and
  `llvm/test/CodeGen/CD/cdbc-machine-parity.list`.
- Modify: the ABI, machine-backend, target README, verification, and two
  roadmap documents.

- [x] Admit `find` only for a proven CD array token, a direct defined
  one-parameter predicate marked by `cd.value.params="0"`, an exact `i1`
  callback result, and an address-space-zero `ptr` native result; keep
...
- [x] Keep `flatMap`, `findIndex`, and `reduce` rejected.

Completed on 2026-08-08. Focused find/predicate lit passed `3/3`; the full
local CD suite passed `87` tests with `1` expected unsupported VM integration
case; direct/machine parity passed `62/62`; parity unit tests passed `14/14`,
...
## Task 10: Extend the callback native lane with `flatMap`
...
- Modify: `llvm/lib/Target/CD/CDValueABI.cpp` and
  `llvm/lib/Target/CD/CDBytecodeFormat.cpp`.
- Create: `llvm/test/CodeGen/CD/cdbc-native-flat-map.ll`,
  `llvm/test/CodeGen/CD/cdbc-native-flat-map-runtime.ll`, and
  `llvm/test/CodeGen/CD/cdbc-native-flat-map-errors.ll`.
- Modify: `llvm/test/CodeGen/CD/cdbc-native-errors.ll` and
  `llvm/test/CodeGen/CD/cdbc-machine-parity.list`.
- Modify: the ABI, machine-backend, target README, verification, and two
  roadmap documents.

- [x] Admit `flatMap` only for a proven CD array token, a direct defined
  one-parameter callback marked by `cd.value.params="0"`, an address-space-zero
  `cd.value.return` callback result, and an exact address-space-zero `ptr`
  native result.
- [x] Reuse `make_function` and `native_call` in both direct and machine paths.
...
## Task 12: Propagate proven dynamic CD values through `select`

This follow-on reuses the existing conditional control-flow and register-move
artifact operations. It adds no opcode, artifact field, `.cdbc 0.1` version, or
nested VM change.
...
- Modify: `llvm/lib/Target/CD/CDValueABI.cpp`,
  ...
  `llvm/lib/Target/CD/CDMachineBytecodeEmitter.cpp`.
- Create: `llvm/test/CodeGen/CD/cdbc-dynamic-select.ll` and
  `llvm/test/CodeGen/CD/cdbc-dynamic-select-errors.ll`.
- Modify: `llvm/test/CodeGen/CD/cdbc-machine-parity.list` and the ABI,
  machine-backend, target README, verification, roadmap, and active-plan docs.
...
that provenance and may cross a marked return or explicit CD consumer. Ordinary
pointers, foreign address spaces, mixed proven/unproven arms, PHI propagation,
and dynamic local storage remain rejected.

- [x] Recognize dynamic `select` results in shared CD provenance validation.
- [x] Reuse direct and machine lowering through existing branch/move artifact
  instructions.
- [x] Cover positive true/false selection, marked function transport, and
  ordinary/foreign/mixed pointer diagnostics with direct/machine parity.
- [x] Keep `cdbc 0.1`, the nested VM checkout, and the existing `select` wire
  representation unchanged.

Completed on 2026-08-08. Focused dynamic-select lit passed `2/2`; the refreshed
...
## Task 17: Extend the bounded native lane with `copy`

This outer-only follow-on reuses the existing `native_call` artifact operation
and the Rust VM's already-supported `copy` helper. It adds no opcode, artifact
field, `.cdbc 0.1` version, or nested VM change.

**Files:**
- Modify: `llvm/lib/Target/CD/CDValueABI.cpp` and
  `llvm/lib/Target/CD/CDBytecodeFormat.cpp`.
- Create: `llvm/test/CodeGen/CD/cdbc-native-copy.ll` and
  `llvm/test/CodeGen/CD/cdbc-native-copy-runtime.ll`.
- Modify: `llvm/test/CodeGen/CD/cdbc-native-errors.ll` and
  `llvm/test/CodeGen/CD/cdbc-machine-parity.list`.
- Modify: the ABI, machine-backend, target README, verification, roadmap, and
  active-plan documents.
...
The collection operand must have proven CD provenance. The Rust VM owns the
runtime array check, snapshot, and fresh shallow-array result semantics.

- [x] Admit `copy` only for a proven CD dynamic-value token and an exact
  address-space-zero `ptr` result.
- [x] Reuse the existing `native_call` bridge in both direct and machine paths.
- [x] Cover positive empty/non-empty copies, runtime non-array failure,
  malformed arity/result/pointer diagnostics, and direct/machine parity.
- [x] Keep future native names rejected and the nested VM checkout unchanged.

Completed on 2026-08-10. Focused copy/error lit passed `3/3`; the full local CD
suite passed `108` tests with `107` passed and `1` expected unsupported VM case;
direct/machine parity passed `77/77`; parity unit tests passed `14/14`,
module-link unit tests passed `5/5`, module-link direct/machine integration
passed, Rust VM tests passed `73 + 3 + 8`, and the nested checkout remained
clean.

## Task 18: Extend the bounded native lane with `concat`

This outer-only follow-on reuses the existing `native_call` artifact operation
and the Rust VM's already-supported `concat` helper. It adds no opcode,
artifact field, `.cdbc 0.1` version, or nested VM change.

**Files:**
- Modify: `llvm/lib/Target/CD/CDValueABI.cpp` and
  `llvm/lib/Target/CD/CDBytecodeFormat.cpp`.
- Create: `llvm/test/CodeGen/CD/cdbc-native-concat.ll` and
  `llvm/test/CodeGen/CD/cdbc-native-concat-runtime.ll`.
- Modify: `llvm/test/CodeGen/CD/cdbc-native-errors.ll` and
  `llvm/test/CodeGen/CD/cdbc-machine-parity.list`.
- Modify: the ABI, machine-backend, target README, verification, roadmap, and
  active-plan documents.
...
Both collection operands must have proven CD provenance. The Rust VM owns the
runtime array checks, snapshot order, and fresh shallow-array result semantics.

- [x] Admit `concat` only for two proven CD dynamic-value tokens and an exact
  address-space-zero `ptr` result.
- [x] Reuse the existing `native_call` bridge in both direct and machine paths.
- [x] Cover positive empty/non-empty ordered concatenation, runtime non-array
  failure, malformed arity/result/pointer diagnostics, and direct/machine parity.
- [x] Keep future native names rejected and the nested VM checkout unchanged.

Completed on 2026-08-10. Focused concat/error lit passed `3/3`; the full local
CD suite passed `110` tests with `109` passed and `1` expected unsupported VM
case; direct/machine parity passed `79/79`; parity unit tests passed `14/14`,
module-link unit tests passed `5/5`, module-link direct/machine integration
passed, Rust VM tests passed `73 + 3 + 8`, and the nested checkout remained
clean.
...
## Task 20: Extend the bounded native lane with `values`

This outer-only follow-on reuses the existing `native_call` artifact operation
and the Rust VM's already-supported `values` helper. It adds no opcode,
artifact field, `.cdbc 0.1` version, or nested VM change.

**Files:**
- Modify: `llvm/lib/Target/CD/CDValueABI.cpp` and
  `llvm/lib/Target/CD/CDBytecodeFormat.cpp`.
- Create: `llvm/test/CodeGen/CD/cdbc-native-map-values.ll` and
  `llvm/test/CodeGen/CD/cdbc-native-values-runtime.ll`.
- Modify: `llvm/test/CodeGen/CD/cdbc-native-errors.ll` and
  `llvm/test/CodeGen/CD/cdbc-machine-parity.list`.
- Modify: the ABI, machine-backend, target README, verification, roadmap, and
  active-plan documents.
...
- [x] Admit `values` only for a proven CD dynamic-value token and an exact
  address-space-zero `ptr` result.
- [x] Reuse the existing `native_call` bridge in both direct and machine paths.
- [x] Cover positive empty/non-empty insertion-ordered value arrays, runtime
  non-map failure, malformed arity/result/pointer diagnostics, and parity.
- [x] Keep future native names rejected and the nested VM checkout unchanged.

Completed on 2026-08-10. Focused values/error lit passed `3/3`; the full local
CD suite passed `114` tests with `113` passed and `1` expected unsupported VM
case; direct/machine parity passed `83/83`; parity unit tests passed `14/14`;
module-link unit tests passed `5/5`; module-link direct/machine integration
...
## Task 24: Extend the bounded native lane with `push`
...
- Modify: `llvm/lib/Target/CD/CDValueABI.cpp` and
  `llvm/lib/Target/CD/CDBytecodeFormat.cpp`.
- Create: `llvm/test/CodeGen/CD/cdbc-native-push.ll` and
  `llvm/test/CodeGen/CD/cdbc-native-push-runtime.ll`.
- Modify: `llvm/test/CodeGen/CD/cdbc-native-errors.ll` and
  `llvm/test/CodeGen/CD/cdbc-machine-parity.list`.
- Modify: the ABI, machine-backend, target README, verification, roadmap, and
  active-plan documents.
...
~~~llvm
llvm.cd.native(ptr name, ptr array, value) -> ptr
~~~

The array operand must have proven CD provenance and the value must be scalar
or a proven CD dynamic value. The Rust VM owns the runtime array check, append
mutation, resource checkpoint, and nil result.

- [x] Admit `push` only for a proven CD dynamic-value array, a scalar or CD
  dynamic-value value, and an exact address-space-zero `ptr` result.
- [x] Reuse the existing `native_call` bridge in both direct and machine paths.
- [x] Cover scalar/CD value append, shared-array mutation, nil result, runtime
  non-array failure, malformed arity/result/pointer diagnostics, and parity.
- [x] Keep future native names rejected and the nested VM checkout unchanged.

Completed on 2026-08-10. Focused push/error lit passed `3/3`; the full local
CD suite passed `123` tests with `122` passed and `1` expected unsupported VM
case; direct/machine parity passed `92/92`; parity unit tests passed `14/14`;
module-link unit tests passed `5/5`; module-link direct/machine integration
passed; Rust VM tests passed `73 + 3 + 8`; and `git diff --check` passed.
