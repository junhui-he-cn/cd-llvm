...
The current local LLVM-only run is `111 passed / 1 unsupported` across 112
fixtures. The direct/machine parity manifest has 81 passing entries, including
the `contains`, `slice`, `copy`, `concat`, and `keys` behavior, selected `map`, `filter`, `flatMap`, `reduce`, `any`,
