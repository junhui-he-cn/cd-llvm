...
The current local LLVM-only run is `115 passed / 1 unsupported` across 116
fixtures. The direct/machine parity manifest has 85 passing entries, including
the `contains`, `slice`, `copy`, `concat`, `remove`, `keys`, and `values` behavior, selected `map`, `filter`, `flatMap`, `reduce`, `any`,
