...
The current local LLVM-only run is `113 passed / 1 unsupported` across 114
fixtures. The direct/machine parity manifest has 83 passing entries, including
the `contains`, `slice`, `copy`, `concat`, `keys`, and `values` behavior, selected `map`, `filter`, `flatMap`, `reduce`, `any`,
