...
The current local LLVM-only run is `109 passed / 1 unsupported` across 110
fixtures. The direct/machine parity manifest has 79 passing entries, including
the `contains`, `slice`, `copy`, and `concat` behavior, selected `map`, `filter`, `flatMap`, `reduce`, `any`,
