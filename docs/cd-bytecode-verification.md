...
The current local LLVM-only run is `87 passed / 1 unsupported` across 88
fixtures. The direct/machine parity manifest has 62 passing entries, including
the selected `map`, `filter`, `any`, `all`, `count`, and `find` callback behavior
and runtime type-error cases.
...
| Callback native `map`, `filter`, `any`, `all`, `count`, and `find` | cdbc-native-map.ll, cdbc-native-map-runtime.ll, cdbc-native-filter.ll, cdbc-native-filter-runtime.ll, cdbc-native-any-all.ll, cdbc-native-any-runtime.ll, cdbc-native-all-runtime.ll, cdbc-native-count.ll, cdbc-native-count-runtime.ll, cdbc-native-find.ll, cdbc-native-find-runtime.ll, cdbc-native-predicate-errors.ll, and callback cases in cdbc-native-errors.ll | Direct/machine output, runtime type errors, empty-array identities, predicate ABI, full count traversal, first-match/no-match `find` behavior, and callback-shape diagnostics passed |
